...
with tab4:
    ...
    if run_perfect_scan:
        ...
        if not market_symbols:
            ...
            with st.spinner("Makro Veri (5 Yıllık) ve Mikro Veri (1 Yıllık) Çoklu Havuzu Oluşturuluyor..."):
                # Eşzamanlı iki havuz çekimi (Multi-pool caching)
            ...
            with st.spinner("Kusursuz Kesişim Matrisi (Hacim Patlaması + Matematiksel Döngü) taranıyor..."):
                ...
                for idx, symbol in enumerate(market_symbols):
                    ...
                    # Her iki veri havuzunda da ilgili sembolün mevcut olup olmadığının kontrolü
                    if (yf_ticker_key in df_weekly_all.columns.levels[0]) and (yf_ticker_key in df_daily_all.columns.levels[0]):
                        ...
                        # Algoritma 1: Haftalık Periyotta Hacim Onaylı Makro Trend Kırılımı
                        is_macro_break, macro_ctx = evaluate_macro_trader_breakout(df_weekly)
                        
                        # Algoritma 2: Günlük Periyotta Profesyonel Otonom Fibonacci Kurulumu
                        is_fib_setup, fib_ctx = evaluate_auto_fibonacci(df_daily)
                        
                        # KESİŞİM FİLTRESİ: İki bağımsız strateji aynı anda onay veriyorsa
    ...
    # Tarama sonuçlarının ekrana basılması (Dataframe rendering)
    if st.session_state.tab4_rows:
        ...
        # Döngü durumuna göre gruplamak için sıralama yapıyoruz
    ...
    elif "run_perfect_scan" in locals() and run_perfect_scan:
