...
with tab1:
    ...
    if execute_scan:
        ...
        if not tv_passed_stocks:
            # HATA DÜZELTİLDİ: st.warning icon hatası markdown ile değiştirildi.
            ...
            # HATA DÜZELTİLDİ: st.info icon hatası markdown ile değiştirildi.
    ...
    if st.session_state.final_rows:
        ...
        st.write("---")
        # HATA DÜZELTİLDİ
        st.markdown(f"<div style='color:#9ca3af; font-family:Inter; font-weight:600;'>[SİSTEM BİLGİSİ] {st.session_state.last_tf} periyodunda belirtilen hacim ve indikatör koşullarını sağlayan hisse bulunamadı.</div>", unsafe_allow_html=True)
...
with tab2:
    ...
    if run_pine_scan:
        ...
        if not bist_symbols:
            ...
            # HATA DÜZELTİLDİ
            st.markdown("<div style='color:#10b981; font-family:Inter; font-weight:600;'>[SİSTEM BİLGİSİ] Algoritma taraması tamamlandı.</div>", unsafe_allow_html=True)
